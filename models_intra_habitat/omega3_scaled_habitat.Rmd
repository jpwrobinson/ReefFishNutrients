---
title: "Omega-3 model - habitat dataset, scaled"
...
## drop c. argus - only 1 sample
focal <- focal %>% filter(species  != 'Cephalopholis argus')
...
print('Priors are:')
print(posts.scaled %>% filter(nut == nut) %>% select(nut,covariate, post, sd)) %>% mutate(post = round(post,3), sd = round(sd,3))
...
nt_m<-ulam(
        alist(
          ...
          log(mu) <- a + a_sp[species] + B3*trophic_level + B4*Lmax + 
                B5*K + B6*tm + B7*benthic_pelagic_dummy + (B8 + B8_sp[species])*habitat_dummy + # fixed
          			X1[site]*sigma_site + X2[family]*sigma_fam, 	# random site & family effect
          ...
          # fixed priors
          a ~ dnorm(0, 100),
          B3 ~ dnorm(0.013, 0.165),
    		  B4 ~ dnorm(-0.174, 0.120),
    		  B5 ~ dnorm(-0.140, 0.436),
    		  B6 ~ dnorm(-0.108, 0.183),
    		  B7 ~ dnorm(-0.032, 0.189),
    		  B8 ~ dnorm(0, 1), 
    		  X1[site] ~ dnorm(0, 1),
    		  X2[family] ~ dnorm(0, 1),
          # vector[2]:sigma_species ~ dexp(1),
          c(sigma_species, sigma_site, sigma_fam) ~ dexp(1),
          scale ~ dcauchy(0, 0.5),
    		  Rho ~ lkj_corr(3)
        ...
        data=focal.fit, iter=5000, warmup=1500, chains=3, cores = 6, control=list(adapt_delta=0.99), message=FALSE)
...
ggplot() + 
            ...
            labs(x='mu', title='Mean omega3 posterior') +       
...
pred<-expand.grid(species = unique(focal$species),
	...
	# replace varying intercept samples with zeros (SITE)
	...
	base<-expand.grid(
						  ...
						  species = unique(focal$species)[3],
						  site = levels(focal$site)[4], 
						  habitat_dummy =c(1,2),
						  trophic_level = 0, 
						  Lmax = 0, 
						  K = 0, 
  ...
  base$habitat<-focal$habitat[match(base$habitat_dummy, focal$habitat_dummy)]
	...
	base$lower50 <- apply( meds , 2 , HPDI , prob=0.50 )[1,]
	base$upper50 <- apply( meds , 2 , HPDI , prob=0.50 )[2,]
	base$nutrient<-nut
	base$species<-'Average effect'
	base
# 	## transform mu
#     mast$mu<-exp(mast$mu)
