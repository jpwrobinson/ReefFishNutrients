...
nt_m<-ulam(
        ...
        data=focal.fit, iter=5000, warmup=1500, chains=3, cores = 6, control=list(adapt_delta=0.99), message=FALSE, log_lik=TRUE)
...
ggplot() + 
            geom_density(aes(((mu.ma))), fill='red', alpha=0.5) + 
          # geom_density(aes(((mu.coral))), fill='grey', alpha=0.5) + 
...
#	geom_rect(data = base, aes(ymin = lower95, ymax = upper95, xmin = -Inf, xmax=Inf), fill='grey', alpha=0.2) +
	...
	labs(y = 'Mineral concentration (g per 100g)', x ='') +
