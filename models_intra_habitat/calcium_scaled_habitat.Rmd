...
focal.fit<-focal %>% select(logvalue, trophic_level, Lmax, K, tm, benthic_pelagic_dummy, habitat_dummy, species, family, site)
```
...
nt_m<-ulam(
        alist(
          ...
          # trait fixed effects
          mu <- a + a_sp[species] + B3*trophic_level + B4*Lmax + 
          ...
          c(sigma, sigma_site, sigma_fam) ~ dexp(1),
        ...
        data=focal.fit, iter=5000, warmup=1500, chains=3, cores = 6, control=list(adapt_delta=0.99), messages=FALSE)
