---
title: "Iron model - full dataset, scaled"
...
print('Priors are:')
print(posts %>% filter(nut == 'Fe') %>% select(nut,covariate, post, sd)) %>% mutate(post = round(post,3), sd = round(sd,3))
...
nt_m<-ulam(
        ...
        data=focal.fit, iter=5000, warmup=1500, chains=3, cores = 4, log_lik=TRUE, control=list(adapt_delta=0.99), messages=FALSE)
...
ggplot(focal.fit, aes(y_post, value, col=family)) + 
        geom_point() + 
...
```{r}
# mod<-stan_model('scripts/stan_code/iron_org.stan')
# foc<-output[[3]] %>% mutate(site = as.numeric(site), family=as.numeric(family))
# foc$benthic.pelagic.dummy<-NULL
# m11.4_stan <- stan( file= 'scripts/stan_code/iron_org.stan', data=foc, chains=4 )
```
