...
focal.fit<-focal %>% select(logvalue, trophic_level, Lmax, K, tm, benthic_pelagic_dummy, site, family)
```
...
nt_m<-ulam(
        alist(
          ...
          c(sigma, sigma_site, sigma_fam) ~ dexp(2)
        ),
        data=focal.fit, iter=5000, warmup=1500, chains=3, cores = 4, log_lik = TRUE, control=list(adapt_delta=0.99))
